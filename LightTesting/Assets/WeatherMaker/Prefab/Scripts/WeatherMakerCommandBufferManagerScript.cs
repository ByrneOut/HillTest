    ...
    public class WeatherMakerCommandBufferManagerScript : MonoBehaviour
    {
        ...
        private void UpdateCommandBuffersForCamera(Camera camera)
        {
            ...
            foreach (WeatherMakerCommandBuffer commandBuffer in commandBuffers)
            {
                ...
                if (commandBuffer.CameraType == WeatherMakerCameraType.CubeMap)
                {
                    if (!commandBuffer.Material.IsKeywordEnabled("WEATHER_MAKER_CUBEMAP"))
                    {
                        commandBuffer.Material.EnableKeyword("WEATHER_MAKER_CUBEMAP");
                    }
                }
                else if (commandBuffer.CameraType == WeatherMakerCameraType.Reflection)
                {
                    if (!commandBuffer.Material.IsKeywordEnabled("WEATHER_MAKER_REFLECTION"))
                    {
                        commandBuffer.Material.EnableKeyword("WEATHER_MAKER_REFLECTION");
                    }
                }
                else
                {
                    if (commandBuffer.Material.IsKeywordEnabled("WEATHER_MAKER_CUBEMAP"))
                    {
                        commandBuffer.Material.DisableKeyword("WEATHER_MAKER_CUBEMAP");
                    }
                    if (commandBuffer.Material.IsKeywordEnabled("WEATHER_MAKER_REFLECTION"))
                    {
                        commandBuffer.Material.DisableKeyword("WEATHER_MAKER_REFLECTION");
                    }
                }
