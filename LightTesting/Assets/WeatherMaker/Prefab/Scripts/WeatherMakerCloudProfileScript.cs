...
namespace DigitalRuby.WeatherMaker
{
    ...
    public class WeatherMakerCloudProfileScript : ScriptableObject
    {
        ...
        [Tooltip("How much clouds effect directional light shadow strength, lower values ensure no reduction.")]
        [Range(0.0f, 3.0f)]
        public float CloudShadowStrength = 1.0f;

        [Tooltip("How much clouds effect directional light intensity, lower values ensure no reduction.")]
        [Range(0.0f, 3.0f)]
        ...
        public void SetShaderCloudParameters(Material m)
        {
            ...
            if (CloudsEnabled)
            {
                ...
                if (Application.isPlaying)
                {
                    ...
                    float sunIntensityMultiplier = Mathf.Clamp(1.0f - (CloudDensityTotal * CloudLightStrength * 0.85f), 0.0f, 1.0f);
                    ...
                    float sunShadowMultiplier = Mathf.Lerp(1.0f, 0.0f, Mathf.Clamp(((CloudDensityTotal + cover) * CloudShadowStrength * 0.85f), 0.0f, 1.0f));
