	...
	SubShader
	{
		...
		Pass
		{
			...
			v2f vert (vertex_uv_normal v)
			{
				...
				o.uv = float4((_WorldSpaceSampleScale ? RotatePointZeroOriginQuaternion(-WorldSpaceViewDir(v.vertex), q).xz * _WorldSpaceSampleScale : uv) + (_AnimationSpeed.xy * _WeatherMakerTime.y), _WeatherMakerTime.y * _AnimationSpeed.z, 0.0);
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				col.rgb = CalculateLightColorWorldSpace(i.worldPos, col.rgb, _WeatherMakerAmbientLightColorGround, 1).rgb;
